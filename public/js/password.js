	...
	$('.update-password-form').submit(function(event){
        ...
        request.done(function(response){
        	if (response.status === 'invalid-reason') {
                ...
                handleButton(button, spinner);
                handleErrors($('.current-password'), $('.current-password-error'), 'Incorrect password.');
            } else if (response.status === 'invalid-password') {
                handleButton(button, spinner);
                handleErrors($('.new-password'), $('.new-password-error'), 'Password must be between 6 - 15 characters.');
